...
public class MyKeyboards {
    ...
    private static Keyboard inlineFromProfileKeyboard;
    ...
    public static Keyboard getStartKeyboard() {
        if (startKeyboard == null) {
            ...
            line2.add(new KeyboardButton().setAction(new KeyboardButtonAction().setLabel("Задания").setType(TemplateActionTypeNames.TEXT)).setColor(KeyboardButtonColor.NEGATIVE));

            line3.add(new KeyboardButton().setAction(new KeyboardButtonAction().setLabel("Пополнить баланс").setType(TemplateActionTypeNames.TEXT)).setColor(KeyboardButtonColor.PRIMARY));
    ...
    public static Keyboard getNotBoughtPhotoKeyboard(int previous, int present, int next) {
        ...
        line1.add(new KeyboardButton().setAction(new KeyboardButtonAction()
                .setLabel("Предыдущее " + previous)
                .setType(TemplateActionTypeNames.CALLBACK)
                .setPayload("{\"button\": \"предыдущее " + previous + "\"}")).setColor(KeyboardButtonColor.PRIMARY));

        line1.add(new KeyboardButton().setAction(new KeyboardButtonAction()
                .setLabel("Приобрести " + present)
                .setType(TemplateActionTypeNames.CALLBACK)
                .setPayload("{\"button\": \"приобрести " + present + "\"}")).setColor(KeyboardButtonColor.POSITIVE));

        line1.add(new KeyboardButton().setAction(new KeyboardButtonAction()
                .setLabel("Следующее " + next)
                .setType(TemplateActionTypeNames.CALLBACK)
                .setPayload("{\"button\": \"следующее " + next + "\"}")).setColor(KeyboardButtonColor.PRIMARY));

        allKey.add(line1);

        buyingPhotoKeyboard.setButtons(allKey);
        buyingPhotoKeyboard.setInline(true);
    ...
    public static Keyboard getBoughtPhotoKeyboard(int previous, int next) {
        ...
        line1.add(new KeyboardButton().setAction(new KeyboardButtonAction()
                .setLabel("Уже куплено")
                .setType(TemplateActionTypeNames.CALLBACK)
                .setPayload("{\"button\": \"куплено\"}")).setColor(KeyboardButtonColor.NEGATIVE));
    ...
    public static Keyboard getMyPhotoKeyboard(int previous, int next) {
        ...
        myPhotoKeyboard.setInline(true);
    ...
    public static Keyboard getInlineFromProfileKeyboard() {
        if (inlineFromProfileKeyboard == null) {
            inlineFromProfileKeyboard = new Keyboard();
            ...
            inlineFromProfileKeyboard.setButtons(allKey);
            inlineFromProfileKeyboard.setInline(true);
        }
        return inlineFromProfileKeyboard;
    }

//    public static Keyboard getTasksKeyboard() {
//        Keyboard tasksKeyboard = new Keyboard();
//
//        List<List<KeyboardButton>> allKey = new ArrayList<>();
//
//        List<KeyboardButton> line1 = new ArrayList<>();
...
//
//
//        line1.add(new KeyboardButton().setAction(new KeyboardButtonAction().setLabel("Подписка на группу").setType(TemplateActionTypeNames.TEXT)).setColor(KeyboardButtonColor.POSITIVE));
//
//
